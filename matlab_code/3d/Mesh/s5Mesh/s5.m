...
%Plot mesh
    if mplot ==1
        ...
        saveas(f2,[folder 'mesh/2dmesh.fig']);
        close;
        f3=figure();
        plot3Elem(Xb,nXb,Nb);
        plot3Elem(Xc,nXc,Nc);
        %title(['lt= ',num2str(lt_),'   lr= ',num2str(lr_), '   bang= ',num2str(bang_),'   hfactor= ', num2str(hfactor), '   wfactor= ', num2str(wfactor)]);
        saveas(f3,[folder 'mesh/3dmesh.fig']);
        close;
    end

    fprintf(fid, 'lt_   = %6.3f, lr_= %6.3f, l_ = %6.3f, bang_  = %6.3f, c_   = %6.3f, hfactor = %6.3f, wfactor = %6.3f\n', ...
            lt_,lr_,l_,bang_,c_,hfactor,wfactor);
